...
public class GenerateOtpService {
    ...
    public void sendOtpNotification(AcmUser user)
    {
        log.debug("Generate OTP notification for user [{}]", user.getUserId());
        String otp = this.generateOtp();

        user.setMfaToken(otp);
        user.setMfaCreatedDateTime(LocalDateTime.now());
        userDao.save(user);
        MDC.put(MDCConstants.EVENT_MDC_REQUEST_USER_ID_KEY, user.getUserId());
        auditPropertyEntityAdapter.setUserId(user.getUserId());

        String emailSubject = "";
        Template template = templateManager.findTemplate("newOtp.html");
        if (template != null)
        {
            emailSubject = template.getEmailSubject();
        }
        //emailSubject = emailSubject.replace("#ACM_OTP#", otp);

        Notification notification = notificationService.getNotificationBuilder()
                .newNotification("newOtp", NotificationConstants.NEW_OTP, "USER", user.getIdentifier(),
                        user.getUserId())
