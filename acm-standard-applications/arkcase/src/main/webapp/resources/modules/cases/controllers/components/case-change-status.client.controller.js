'use strict';

angular.module('cases').controller(
        'Cases.ChangeStatusController',
        [ '$scope', '$http', '$stateParams', '$translate', '$modalInstance', 'Complaint.InfoService', '$state', 'Object.LookupService', 'MessageService', 'UtilService', '$modal', 'ConfigService', 'ObjectService', 'modalParams', 'Case.InfoService', 'Object.ParticipantService','Admin.FormWorkflowsLinkService', 'Object.ModelService', 'Profile.UserInfoService','Helper.NoteService', 'Object.NoteService', 'Authentication', '$q',
                function($scope, $http, $stateParams, $translate, $modalInstance, ComplaintInfoService, $state, ObjectLookupService, MessageService, Util, $modal, ConfigService, ObjectService, modalParams, CaseInfoService, ObjectParticipantService, AdminFormWorkflowsLinkService, ObjectModelService, UserInfoService,  HelperNoteService, ObjectNoteService, Authentication, $q) {

                    $scope.modalParams = modalParams;
                    $scope.currentStatus = modalParams.info.status;
                    $scope.oInfo = modalParams.objectInfo;
                    $scope.approverName = "";
                    $scope.groupName = "";
                    //Functions
                    $scope.statusChanged = statusChanged;
                    $scope.save = save;
                    $scope.cancelModal = cancelModal;
                    //Objects
                    $scope.showCaseCloseStatus = false;
                    $scope.showApprover = false;//modalParams.showApprover;
                    $scope.changeCaseStatus = {
                        caseId: modalParams.info.caseId,
                        status: "",
                        caseResolution: "",
                        objectType: "CHANGE_CASE_STATUS",
                        changeDate: new Date(),
                        participants: [],
                        created: null,
                        creator: null,
                        modified: null,
                        modifier: null,
                        assignee: null,
                        description: "",
                        changeCaseStatusFlow: $scope.showApprover == 'true'
                    };

                    $scope.note = {};
                    $scope.note.note = "";

                    ConfigService.getComponentConfig("cases", "notes").then(function (config) {
                     $scope.notesInit = {
                            objectType: ObjectService.ObjectTypes.CASE_FILE,
                            currentObjectId: $stateParams.id
                         };
                     });

                      var noteHelper = new HelperNoteService.Note();
                     $scope.notesInit = {
                          noteTitle: $translate.instant("cases.comp.notes.title"),
                          objectType: ObjectService.ObjectTypes.CASE_FILE,
                          currentObjectId: $stateParams.id,
                          parentTitle: "",
                          noteType: "GENERAL"
                     };

                     //var userId = "";
                      Authentication.queryUserInfo().then(function(userInfo) {
                          var userId = userInfo.userId;
                          var info = $scope.notesInit;
                          $scope.note = noteHelper.createNote(info.currentObjectId, info.objectType, $scope.oInfo.caseNumber, info.tag, userId );

                         return userInfo;
                     });


                    var participantTypeApprover = 'approver';
                    var participantTypeOwningGroup = "owning group";

                    ConfigService.getModuleConfig("cases").then(function(moduleConfig) {
                        $scope.futureTaskConfig = _.find(moduleConfig.components, {
                            id: "newFutureTask"
                        });
                    });

                    ConfigService.getComponentConfig("cases", "participants").then(function(componentConfig) {
                        $scope.participantsConfig = componentConfig;
                    });

                    ObjectLookupService.getLookupByLookupName("changeCaseStatuses").then(function(caseStatuses) {
                        $scope.statuses = [];
                        var defaultChangeCaseStatus = ObjectLookupService.getPrimaryLookup($scope.statuses);
                        if (defaultChangeCaseStatus && !$scope.changeCaseStatus.status) {
                            $scope.changeCaseStatus.status = defaultChangeCaseStatus.key;
                        }
                        UserInfoService.getUserInfo().then(function(infoData) {
                            $scope.currentUserProfile = infoData;
                            $scope.isCms = $scope.currentUserProfile.groups[0] === "CMS@APVITACMS.COM";
                            if($scope.isCms){
                                for(var i = 0; i < caseStatuses.length; i++){
                                    if(caseStatuses[i].key === 'CMS Approved'
                                        || caseStatuses[i].key === 'CMS Approved-Documentation Pending'
                                        || caseStatuses[i].key === 'CMS Pending- On Hold'
                                        || caseStatuses[i].key === 'CMS Requested Edits'){
                                        $scope.statuses.push(caseStatuses[i]);
                                    }
                                }
                            } else {
                                 if($scope.oInfo.caseType === 'NLM' || $scope.oInfo.caseType === 'LCM') {
                                     if($scope.oInfo.caseAdminActionsOutcome !== 'DEAC'){
                                         for(var i = 0; i < caseStatuses.length; i++){
                                            if(caseStatuses[i].value === 'Case Closed – MAC Deactivation Referral'){
                                              caseStatuses.splice(i, 1);
                                            }
                                          }
                                     }

                                 } else {
                                    for(var i = 0; i < caseStatuses.length; i++){
                                         if(caseStatuses[i].value === 'Case Closed – MAC Deactivation Referral'){
                                           caseStatuses.splice(i, 1);
                                         }
                                     }
                                 }

                                  if($scope.oInfo.caseType === 'OPT') {
                                    for(var i = 0; i < caseStatuses.length; i++){
                                        if(caseStatuses[i].value === 'NON-OPT Case - Non-Actionable'){
                                             caseStatuses.splice(i, 1);
                                        }
                                        if(caseStatuses[i].value === 'Documentation Requested'){
                                             caseStatuses.splice(i, 1);
                                        }
                                        if(caseStatuses[i].value === 'CMS Approved-Documentation Pending'){
                                             caseStatuses.splice(i, 1);
                                        }
                                        if(caseStatuses[i].value === 'Submitted to CMS-Documentation Pending'){
                                             caseStatuses.splice(i, 1);
                                        }
                                        if(caseStatuses[i].value === 'Resubmitted To CMS - Documentation Pending'){
                                             caseStatuses.splice(i, 1);
                                        }
                                    }
                                } else if ($scope.oInfo.caseType !== 'OPT') {
                                     for(var i = 0; i < caseStatuses.length; i++){
                                         if(caseStatuses[i].value === 'OPT - DEX SENT - Docket Requested'){
                                           caseStatuses.splice(i, 1);
                                         }
                                         if(caseStatuses[i].value === 'OPT Case - Non-Actionable'){
                                               caseStatuses.splice(i, 1);
                                         }
                                     }
                                     if ($scope.oInfo.caseType === 'MED') {
                                         for(var i = 0; i < caseStatuses.length; i++){
                                             if(caseStatuses[i].value === 'Documentation Requested'){
                                                  caseStatuses.splice(i, 1);
                                             }
                                         }
                                     }

                                }

                               // console.log("!!!!! ", caseStatuses);
                                $scope.statuses = caseStatuses;
                            }
                        });
                    });

                    function statusChanged() {
                        var currentDate = new Date().toJSON().slice(0, 10);
                        console.log("!!! curdate: ", currentDate);


                        $scope.showCaseCloseStatus = $scope.changeCaseStatus.status === "CLOSED";
                        $scope.showCaseCloseStatus = $scope.changeCaseStatus.status === "CLOSED";
                        $scope.disableMessages = [];
                        $scope.disableSave = false;

                        console.log("!!! $scope.oInfo.caseNotActionableReason: ", $scope.oInfo.caseNotActionableReason);
                        console.log("!!! $scope.oInfo.caseAdminActionsOutcome: ", $scope.oInfo.caseAdminActionsOutcome);
                        console.log("!!! $scope.oInfo.caseFinalOutAdminAct: ", $scope.oInfo.caseFinalOutAdminAct);
                        console.log("!!! $scope.oInfo.caseFinalOutActionDate: ", $scope.oInfo.caseFinalOutActionDate);
                        console.log("!!! $scope.changeCaseStatus.status: ", $scope.changeCaseStatus.status);
                        console.log("!!! $scope.oInfo.caseFinalOutAdminAct === 'RCCC' :  ", $scope.oInfo.caseFinalOutAdminAct === "RCCC");

                        if($scope.oInfo.caseType !== "OPT"
                        && ($scope.changeCaseStatus.status === "Audit Completed"
                              || $scope.changeCaseStatus.status === "Audit Assigned"
                              || $scope.changeCaseStatus.status === "Audit N/A"
                              || $scope.changeCaseStatus.status === "CASE_CLOSED"
                              || $scope.changeCaseStatus.status === "CCMD")){


                            //Recommended Outcome of Admin Actions - caseAdminActionsOutcome
                            if($scope.oInfo.caseAdminActionsOutcome ===  null || $scope.oInfo.caseAdminActionsOutcome ===  ""){
                                //console.log("!!! 3");
                                $scope.disableMessages.push("Recommended Outcome of Admin Actions must be entered");
                            }

                            //Final Outcome of Admin Actions - caseFinalOutAdminAct
                            if($scope.oInfo.caseFinalOutAdminAct === null || $scope.oInfo.caseFinalOutAdminAct === "" || $scope.oInfo.caseFinalOutAdminAct === "undefined"){
                                //console.log("!!! 4");
                                $scope.disableMessages.push("Final Outcome of Admin Actions must be entered");
                            }


                              if($scope.oInfo.caseAdminActionsOutcome === "NA"){
                                 //Not Actionable Reason  - caseNotActionableReason
                                     if($scope.oInfo.caseNotActionableReason === null || $scope.oInfo.caseNotActionableReason === "" || $scope.oInfo.caseNotActionableReason === "undefined"){
                                         $scope.disableMessages.push("Not Actionable Reason must be entered");
                                     }
                                 }

                                 //Action Date - caseFinalOutActionDate
                                 if( $scope.oInfo.caseFinalOutActionDate === null || $scope.oInfo.caseFinalOutActionDate === "" || $scope.oInfo.caseFinalOutActionDate === "undefined"){
                                     $scope.disableMessages.push("Action Date must be entered");
                                 }

                                //Recommended Length of Enrollment Bar - caseRecLengthEnrollBar
                                if( $scope.oInfo.caseRecLengthEnrollBar === null || $scope.oInfo.caseRecLengthEnrollBar === "" || $scope.oInfo.caseRecLengthEnrollBar === "undefined"){
                                   $scope.disableMessages.push("Recommended Length of Enrollment Bar must be entered");

                                }

                                //Recommended Effective Date - caseRecEffDate
                                if( $scope.oInfo.caseRecEffDate === null || $scope.oInfo.caseRecEffDate === "" || $scope.oInfo.caseRecEffDate === "undefined"){
                                   $scope.disableMessages.push("Recommended Effective Date must be entered");

                                }

                                //Authorities Cited in Final Letter - caseRevAuthCitedActionLetter
                                if( $scope.oInfo.caseRevAuthCitedActionLetter === null || $scope.oInfo.caseRevAuthCitedActionLetter === "" || $scope.oInfo.caseRevAuthCitedActionLetter === "undefined"){
                                   $scope.disableMessages.push("Authorities Cited in Final Letter must be entered");

                                }

                                //Length of Re-Enrollment Bar - caseLengthReEnrollBar
                                if( $scope.oInfo.caseLengthReEnrollBar === null || $scope.oInfo.caseLengthReEnrollBar === "" || $scope.oInfo.caseLengthReEnrollBar === "undefined"){
                                   $scope.disableMessages.push("Length of Re-Enrollment Bar must be entered");

                                }

                                //Effective Date - caseRevEffActionDate
                                if( $scope.oInfo.caseRevEffActionDate === null || $scope.oInfo.caseRevEffActionDate === "" || $scope.oInfo.caseRevEffActionDate === "undefined"){
                                   $scope.disableMessages.push("Effective Date must be entered");

                                }

                                //OPT CMS Decision ( SHOULD BE LEFT BLANK)  - caseOptCmsDecision
                                if( !($scope.oInfo.caseOptCmsDecision === null || $scope.oInfo.caseOptCmsDecision === "" || $scope.oInfo.caseOptCmsDecision === "undefined")){
                                   $scope.disableMessages.push("OPT CMS Decision must be blank");
                                }

                              }

                      else if($scope.oInfo.caseType !== "OPT" && ($scope.changeCaseStatus.status === "CMS Approved"
                               || $scope.changeCaseStatus.status === "NON-OPT Case - Non-Actionable"
                               || $scope.changeCaseStatus.status === "CMS Requested Edits")){


                            //Recommended Outcome of Admin Actions - caseAdminActionsOutcome
                            if($scope.oInfo.caseAdminActionsOutcome ===  null || $scope.oInfo.caseAdminActionsOutcome ===  ""){
                                //console.log("!!! 3");
                                $scope.disableMessages.push("Recommended Outcome of Admin Actions must be entered");
                            }

                            //Final Outcome of Admin Actions - caseFinalOutAdminAct
                            if($scope.oInfo.caseFinalOutAdminAct === null || $scope.oInfo.caseFinalOutAdminAct === "" || $scope.oInfo.caseFinalOutAdminAct === "undefined"){
                                //console.log("!!! 4");
                                $scope.disableMessages.push("Final Outcome of Admin Actions must be entered");
                            }


                            /*if($scope.changeCaseStatus.status === "NON-OPT Case - Non-Actionable"){*/
                                           //Not Actionable - NA
                                            if($scope.oInfo.caseFinalOutAdminAct === "NA"){
                                               //Authorities Cited in Final Letter - caseRevAuthCitedActionLetter
                                                if($scope.oInfo.caseRevAuthCitedActionLetter !== "N/A"){
                                                    $scope.disableMessages.push("Authorities Cited in Action Letter must be N/A");
                                                }

                                               //Length of Re-Enrollment Bar - caseLengthReEnrollBar
                                               if($scope.oInfo.caseLengthReEnrollBar !== "N/A"){
                                                   $scope.disableMessages.push("Length of Re-Enrollment Bar must be N/A");
                                               }

                                               //Effective Date - caseRevEffActionDate
                                               if($scope.oInfo.caseRevEffActionDate !== "N/A"){
                                                   $scope.disableMessages.push("Effective Date must be N/A");
                                               }

                                                //Action Date - caseFinalOutActionDate
                                                if( $scope.oInfo.caseFinalOutActionDate === null || $scope.oInfo.caseFinalOutActionDate === "" || $scope.oInfo.caseFinalOutActionDate === "undefined"){
                                                  $scope.disableMessages.push("Action Date must be set to today's date");
                                                }

                                               //OPT CMS Decision ( SHOULD BE LEFT BLANK)  - caseOptCmsDecision
                                               if( !($scope.oInfo.caseOptCmsDecision === null || $scope.oInfo.caseOptCmsDecision === "" || $scope.oInfo.caseOptCmsDecision === "undefined")){
                                                    $scope.disableMessages.push("OPT CMS Decision must be blank");
                                               }
                                            }
                                    /*     }*/


                                        //Final Outcome of Admin Actions - caseFinalOutAdminAct

                                         //Consolidated Revocation & Preclusion List - CRPL   - 1
                                         //Revocation Only - RO  - 1
                                         //Preclusion List Only - PLO -1
                                         //Referred to CMS for Compliance Call - RCCC
                                         //Refer to CMS – PECOS Felony Criminal Conviction - PFCC
                                         //Deactivation  - DEAC
                                         //Case Requires Further Information from State Medicaid Agency - CRFISMA
                                         //Other - OTH
                                         //Publish Termination & Refer Termination to DEA - PTRTD
                                         //Do Not Publish Termination & Refer Termination to DEA - DNPTRTD
                                         //Publish Termination & Do Not Refer Termination to DEA - PTDNRTD
                                         //Do Not Publish Termination & Do Not Refer Termination to DEA - DNPT
                                         if($scope.oInfo.caseFinalOutAdminAct === "CRPL"
                                           || $scope.oInfo.caseFinalOutAdminAct === "RO"
                                           || $scope.oInfo.caseFinalOutAdminAct === "PLO"
                                           || $scope.oInfo.caseFinalOutAdminAct === "RCCC"
                                           || $scope.oInfo.caseFinalOutAdminAct === "PFCC"
                                           || $scope.oInfo.caseFinalOutAdminAct === "DEAC"
                                           || $scope.oInfo.caseFinalOutAdminAct === "CRFISMA"
                                           || $scope.oInfo.caseFinalOutAdminAct === "OTH"
                                           || $scope.oInfo.caseFinalOutAdminAct === "PTRTD"
                                           || $scope.oInfo.caseFinalOutAdminAct === "DNPTRTD"
                                           || $scope.oInfo.caseFinalOutAdminAct === "PTDNRTD"
                                           || $scope.oInfo.caseFinalOutAdminAct === "DNPT"
                                           ){

                                            console.log("!!!! $scope.oInfo.caseFinalOutAdminAct === 'RCCC' :  ", $scope.oInfo.caseFinalOutAdminAct === "RCCC");
                                            //Authorities Cited in Final Letter - caseRevAuthCitedActionLetter
                                             if( $scope.oInfo.caseRevAuthCitedActionLetter === null || $scope.oInfo.caseRevAuthCitedActionLetter === "" || $scope.oInfo.caseRevAuthCitedActionLetter === "undefined"){
                                                 $scope.disableMessages.push("Authorities Cited in Final Letter must be entered");
                                             }

                                            //Length of Re-Enrollment Bar - caseLengthReEnrollBar
                                            if( $scope.oInfo.caseLengthReEnrollBar === null || $scope.oInfo.caseLengthReEnrollBar === "" || $scope.oInfo.caseLengthReEnrollBar === "undefined"){
                                                $scope.disableMessages.push("Length of Re-Enrollment Bar must be entered");
                                            }

                                            //Effective Date - caseRevEffActionDate
                                            if( $scope.oInfo.caseRevEffActionDate === null || $scope.oInfo.caseRevEffActionDate === "" || $scope.oInfo.caseRevEffActionDate === "undefined"){
                                                $scope.disableMessages.push("Effective Date must be entered");
                                            }

                                            if($scope.oInfo.caseFinalOutAdminAct === "CRPL"
                                              || $scope.oInfo.caseFinalOutAdminAct === "RO"
                                              || $scope.oInfo.caseFinalOutAdminAct === "PLO"){

                                                 //Action Date - caseFinalOutActionDate
                                                 if( $scope.oInfo.caseFinalOutActionDate === null || $scope.oInfo.caseFinalOutActionDate === "" || $scope.oInfo.caseFinalOutActionDate === "undefined"){
                                                   $scope.disableMessages.push("Action Date must be entered");
                                                 }

                                            }

                                            if ($scope.oInfo.caseFinalOutAdminAct === "RCCC"
                                              || $scope.oInfo.caseFinalOutAdminAct === "PFCC"
                                              || $scope.oInfo.caseFinalOutAdminAct === "DEAC"
                                              || $scope.oInfo.caseFinalOutAdminAct === "CRFISMA"
                                              || $scope.oInfo.caseFinalOutAdminAct === "OTH"
                                              || $scope.oInfo.caseFinalOutAdminAct === "PTRTD"
                                              || $scope.oInfo.caseFinalOutAdminAct === "DNPTRTD"
                                              || $scope.oInfo.caseFinalOutAdminAct === "PTDNRTD"
                                              || $scope.oInfo.caseFinalOutAdminAct === "DNPT"){

                                                if($scope.oInfo.caseFinalOutActionDate === null
                                                    || $scope.oInfo.caseFinalOutActionDate === ""
                                                    || $scope.oInfo.caseFinalOutActionDate === "undefined" ){

                                                      $scope.disableMessages.push("Action Date must be set to today's date");

                                                } else {

                                                      var dateString = $scope.oInfo.caseFinalOutActionDate.toString();
                                                      var formattedActionDate = dateString.split("T")[0];
                                                      var curDateString = currentDate.toString();

                                                      //Action Date - caseFinalOutActionDate
                                                       if(formattedActionDate !== curDateString){

                                                         $scope.disableMessages.push("Action Date must be set to today's date");
                                                       }
                                                }
                                        }

                                        //OPT CMS Decision ( SHOULD BE LEFT BLANK)  - caseOptCmsDecision
                                        if( !($scope.oInfo.caseOptCmsDecision === null || $scope.oInfo.caseOptCmsDecision === "" || $scope.oInfo.caseOptCmsDecision === "undefined")){
                                             $scope.disableMessages.push("OPT CMS Decision must be blank");
                                        }
                                     }
                                } else if($scope.oInfo.caseType !== "OPT" && ($scope.changeCaseStatus.status === "Ready For Review"
                                        || $scope.changeCaseStatus.status === "Ready For Review II")){

                            //Recommended Outcome of Admin Actions - caseAdminActionsOutcome
                            if($scope.oInfo.caseAdminActionsOutcome ===  null || $scope.oInfo.caseAdminActionsOutcome ===  ""){
                                //console.log("!!! 3");
                                $scope.disableMessages.push("Recommended Outcome of Admin Actions must be entered");
                            }

                                    // Recommended
                                    //Not Actionable NA
                                    //Consolidated Revocation & Preclusion List - CRPL   - 1
                                    //Revocation Only - RO  - 1
                                    //Preclusion List Only - PLO -1
                                    //Referred to CMS for Compliance Call - RCCC
                                    //Refer to CMS – PECOS Felony Criminal Conviction - PFCC
                                    //Deactivation  - DEAC
                                    //Case Requires Further Information from State Medicaid Agency - CRFISMA
                                    //Other - OTH
                                    //Publish Termination & Refer Termination to DEA - PTRTD
                                    //Do Not Publish Termination & Refer Termination to DEA - DNPTRTD
                                    //Publish Termination & Do Not Refer Termination to DEA - PTDNRTD
                                    //Do Not Publish Termination & Do Not Refer Termination to DEA - DNPT

                                    if ($scope.oInfo.caseAdminActionsOutcome === "CRPL"
                                        || $scope.oInfo.caseAdminActionsOutcome === "RO"
                                        || $scope.oInfo.caseAdminActionsOutcome === "PLO"
                                        || $scope.oInfo.caseAdminActionsOutcome === "DEAC"
                                        || $scope.oInfo.caseAdminActionsOutcome === "CRFISMA"
                                        || $scope.oInfo.caseAdminActionsOutcome === "OTH"
                                        || $scope.oInfo.caseAdminActionsOutcome === "PTRTD"
                                        || $scope.oInfo.caseAdminActionsOutcome === "DNPTRTD"
                                    ){

                                        //caseRecLengthEnrollBar  $scope.disableMessages.push("OPT CMS Decision must be blank");
                                        if($scope.oInfo.caseRecLengthEnrollBar === null || $scope.oInfo.caseRecLengthEnrollBar === "" || $scope.oInfo.caseRecLengthEnrollBar === "undefined"){
                                            $scope.disableMessages.push("Recommended Length of Enrollment Bar must be entered");

                                        }

                                        //caseRecAuthCitedActionLetter
                                        if($scope.oInfo.caseRecAuthCitedActionLetter === null || $scope.oInfo.caseRecAuthCitedActionLetter === "" || $scope.oInfo.caseRecAuthCitedActionLetter === "undefined"){
                                            $scope.disableMessages.push("Recommended Authorities Cited in Action Letter must be entered");

                                        }
                                           //caseRecEffDate
                                        if($scope.oInfo.caseRecEffDate === null || $scope.oInfo.caseRecEffDate === "" || $scope.oInfo.caseRecEffDate === "undefined"){
                                            $scope.disableMessages.push("Recommended Effective Date must be entered");

                                        }

                                    }

                                    if ($scope.oInfo.caseAdminActionsOutcome === "NA"
                                        || $scope.oInfo.caseAdminActionsOutcome === "RCCC"
                                        || $scope.oInfo.caseAdminActionsOutcome === "PFCC"
                                        || $scope.oInfo.caseAdminActionsOutcome === "PTDNRTD"
                                        || $scope.oInfo.caseAdminActionsOutcome === "DNPT"
                                    ){

                                        //caseRecLengthEnrollBar  $scope.disableMessages.push("OPT CMS Decision must be blank");
                                        if($scope.oInfo.caseRecLengthEnrollBar !== "N/A"){
                                            $scope.disableMessages.push("Recommended Length of Enrollment Bar must be N/A");

                                        }

                                        //caseRecAuthCitedActionLetter
                                        if($scope.oInfo.caseRecAuthCitedActionLetter  !== "N/A") {
                                            $scope.disableMessages.push("Recommended Authorities Cited in Action Letter must be N/A");

                                        }
                                        //caseRecEffDate
                                        if($scope.oInfo.caseRecEffDate  !== "N/A") {
                                            $scope.disableMessages.push("Recommended Effective Date must be N/A");

                                        }

                                    }

                                }

                                if($scope.disableMessages.length > 0){
                                    $scope.disableSave = true;
                                }
                    }


                    // ---------------------------            approver         --------------------------------------
                    $scope.userOrGroupSearch = function() {
                        var params = {};
                        params.header = $translate.instant("cases.comp.change.status.approver.pickerModal.header");
                        params.filter = "fq=\"object_type_s\":(GROUP OR USER)&fq=\"status_lcs\":(ACTIVE OR VALID)";
                        params.extraFilter = "&fq=\"name\": ";
                        params.config = Util.goodMapValue($scope.participantsConfig, "dialogUserPicker");
                        params.secondGrid = 'true';

                        var modalInstance = $modal.open({
                            templateUrl: "directives/core-participants/participants-user-group-search.client.view.html",
                            controller: [ '$scope', '$modalInstance', 'params', function($scope, $modalInstance, params) {
                                $scope.modalInstance = $modalInstance;
                                $scope.header = params.header;
                                $scope.filter = params.filter;
                                $scope.config = params.config;
                                $scope.secondGrid = params.secondGrid;
                                $scope.extraFilter = params.extraFilter;
                            } ],
                            animation: true,
                            size: 'lg',
                            backdrop: 'static',
                            resolve: {
                                params: function() {
                                    return params;
                                }
                            }
                        });

                        modalInstance.result.then(function(selection) {
                            if (selection) {
                                var selectedObjectType = selection.masterSelectedItem.object_type_s;
                                if (selectedObjectType === 'USER') { // Selected user
                                    var selectedUser = selection.masterSelectedItem;
                                    var selectedGroup = selection.detailSelectedItems;

                                    $scope.approverName = selectedUser.name;
                                    addParticipantInChangeCase(participantTypeApprover, selectedUser.object_id_s);

                                    if (selectedGroup) {
                                        $scope.groupName = selectedGroup.name;
                                        addParticipantInChangeCase(participantTypeOwningGroup, selectedGroup.object_id_s);
                                    }

                                    return;
                                } else if (selectedObjectType === 'GROUP') { // Selected group
                                    var selectedUser = selection.detailSelectedItems;
                                    var selectedGroup = selection.masterSelectedItem;

                                    $scope.groupName = selectedGroup.name;
                                    addParticipantInChangeCase(participantTypeOwningGroup, selectedGroup.object_id_s);

                                    if (selectedUser) {
                                        $scope.approverName = selectedUser.name;
                                        addParticipantInChangeCase(participantTypeApprover, selectedUser.object_id_s);
                                    }

                                    return;
                                }
                            }

                        }, function() {
                            // Cancel button was clicked.
                            return [];
                        });

                    };

                    $scope.updateParticipants = function(participantType, participantLdapId) {
                        var len = $scope.oInfo.participants.length;
                        for (var i = 0; i < len; i++) {
	                    if($scope.oInfo.participants[i].participantType =='assignee' || $scope.oInfo.participants[i].participantType =='owning group'){
                                $scope.oInfo.participants[i].replaceChildrenParticipant = true;
                             }
                        }

                          var newParticipant = {};
                        newParticipant.className = $scope.participantsConfig.className;
                        newParticipant.participantType = participantType;
                        newParticipant.participantLdapId = participantLdapId;

                        if (ObjectParticipantService.validateParticipants([newParticipant], true)) {
                            var participantExists = false;
                            _.forEach($scope.oInfo.participants, function (participant) {
                                if(participant.participantType == participantType){
                                    participantExists = true;
                                    participant.participantLdapId = newParticipant.participantLdapId;
                                    participant.replaceChildrenParticipant = true;

                                    return false;
                                }
                            });
                            if(!participantExists){
                                $scope.oInfo.participants.push(newParticipant);
                            }
                            if ($scope.changeCaseStatus.participants.length === 0) {
                                 $scope.changeCaseStatus.participants.push(newParticipant);
                            }
                        }

                    }

                    function addParticipantInChangeCase(participantType, participantLdapId){
                        var newParticipant = {};
                        newParticipant.className = $scope.participantsConfig.className;
                        newParticipant.participantType = participantType;
                        newParticipant.participantLdapId = participantLdapId;

                        if (ObjectParticipantService.validateParticipants([newParticipant], true)) {
                            var participantExists = false;
                            _.forEach($scope.changeCaseStatus.participants, function (participant) {
                                if(participant.participantType == participantType){
                                    participantExists = true;
                                    participant.participantLdapId = newParticipant.participantLdapId;
                                    participant.replaceChildrenParticipant = true;
                                    return false;
                                }
                            });
                            if(!participantExists){
                                $scope.changeCaseStatus.participants.push(newParticipant);
                            }
                        }
                    }

                    function save() {
                        var current_assignee = ObjectModelService.getAssignee($scope.oInfo);
                        var prev_analyst = $scope.oInfo.casePrevAnalyst;
                        $scope.loading = true;
                        $scope.displayCMSanalystErrorMessage = false;
                        $scope.loadingIcon = "fa fa-circle-o-notch fa-spin";
                        var domain = $translate.instant("cases.comp.change.status.domain");

                        if ($scope.changeCaseStatus.status === "Assigned" || $scope.changeCaseStatus.status === "In Process" || $scope.changeCaseStatus.status === "Documentation Requested" ) {
                            if(ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'analysttest@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'supervisor@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'cmsassignmentuser'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'cms_testaccount@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'qaassignmentuser@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'qacasearchiveuser@'.concat(domain.toLowerCase())) {
                                $scope.oInfo.casePrevAnalyst = ObjectModelService.getAssignee($scope.oInfo);
                                ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevAnalyst);
                                ObjectModelService.setGroup($scope.oInfo, 'ALA_ANALYST@'.concat(domain));
                            }

                        } else if ($scope.changeCaseStatus.status === "Ready For Review" || $scope.changeCaseStatus.status === "Ready For Review II") {
                            if(ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'analysttest@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'supervisor@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'cmsassignmentuser@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'cms_testaccount@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'qaassignmentuser@'.concat(domain.toLowerCase())
                            && ObjectModelService.getAssignee($scope.oInfo).toLowerCase() !== 'qacasearchiveuser@'.concat(domain.toLowerCase())) {
                                 $scope.oInfo.casePrevAnalyst = ObjectModelService.getAssignee($scope.oInfo);
                            }
                              ObjectModelService.setAssignee($scope.oInfo, 'supervisor@'.concat(domain.toLowerCase()));
                              ObjectModelService.setGroup($scope.oInfo, 'ALA_SUPERVISOR@'.concat(domain));

                        }
                         else if ($scope.changeCaseStatus.status === "Returned For Revision" || $scope.changeCaseStatus.status === "OPT Case - Non-Actionable"
                                    || $scope.changeCaseStatus.status === "Returned For Revision II" || $scope.changeCaseStatus.status === "NON-OPT Case - Non-Actionable"
                                    || $scope.changeCaseStatus.status === "OPT - DEX SENT - Docket Requested" ) {
                             ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevAnalyst);
                             ObjectModelService.setGroup($scope.oInfo, 'ALA_ANALYST@'.concat(domain));

                        }
                        else  if ($scope.changeCaseStatus.status === "Review Approved" || $scope.changeCaseStatus.status === "Review Approved II" ) {
                             ObjectModelService.setAssignee($scope.oInfo, 'supervisor@'.concat(domain.toLowerCase()));
                             ObjectModelService.setGroup($scope.oInfo, 'ALA_SUPERVISOR@'.concat(domain));

                        } else if ($scope.changeCaseStatus.status === "Submitted to CMS"
                                || $scope.changeCaseStatus.status === "Submitted To CMS II"
                                || $scope.changeCaseStatus.status === "Submitted to CMS-Documentation Pending" ) {
                            ObjectModelService.setAssignee($scope.oInfo, 'cmsassignmentuser@'.concat(domain.toLowerCase()));
                            ObjectModelService.setGroup($scope.oInfo, 'CMS@'.concat(domain));
                            $scope.oInfo.priority = "CMS";

                        }
                         else if ($scope.changeCaseStatus.status === "Resubmitted To CMS"
                               || $scope.changeCaseStatus.status === "Resubmitted To CMS - Documentation Pending") {
                           //The CMS analyst who is assigned the case
                          // console.log("casePrevCMSAnalyst: " + $scope.oInfo.casePrevCMSAnalyst);
                           //$scope.oInfo.casePrevCMSAnalyst

                           if($scope.oInfo.casePrevCMSAnalyst === null || $scope.oInfo.casePrevCMSAnalyst === "") {
                              $scope.loading = false;
                              $scope.displayCMSanalystErrorMessage = true;
                           } else {
                               ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevCMSAnalyst);
                               ObjectModelService.setGroup($scope.oInfo, 'CMS@'.concat(domain));
                               $scope.oInfo.priority = "CMS";
                           }


                        }  else if ($scope.changeCaseStatus.status === "R&R On Pending Case"
                                    || $scope.changeCaseStatus.status === "R&R On Approved Case"
                                    || $scope.changeCaseStatus.status === "Submitted to CMS - ORR (Original Recommendation Retracted)") {
                             //The CMS analyst who is assigned the case
                            // console.log("casePrevCMSAnalyst: " + $scope.oInfo.casePrevCMSAnalyst);
                              if($scope.oInfo.casePrevCMSAnalyst === null || $scope.oInfo.casePrevCMSAnalyst === "") {
                                   $scope.loading = false;
                                   $scope.displayCMSanalystErrorMessage = true;
                              } else {
                                 ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevCMSAnalyst);
                                 ObjectModelService.setGroup($scope.oInfo, 'CMS@'.concat(domain));
                              }

                        } else if ($scope.changeCaseStatus.status === "CMS Pending- On Hold" ){
                            ObjectModelService.setAssignee($scope.oInfo, 'cms_testaccount@'.concat(domain.toLowerCase()));
                            ObjectModelService.setGroup($scope.oInfo, 'CMS@'.concat(domain));

                        } else if ($scope.changeCaseStatus.status === "CMS Requested Edits") {
                            var numberOfDaysToAdd = 0;
                            var holidays = ["12/31", "1/17", "2/21", "5/30", "6/20","7/4", "9/15", "10/10", "11/11", "11/24", "12/25"];

                            if($scope.oInfo.caseType === "OPT" || $scope.oInfo.caseType === "MED"){
                                 numberOfDaysToAdd = 3;
                            } else {
                                 numberOfDaysToAdd = 5;
                            }
                            var newDate = new Date();
                            var Sunday = 0;
                            var Saturday = 6;
                            var daysRemaining = numberOfDaysToAdd;
                            var counter = 0;

                            while (daysRemaining > 0) {
                                 var someDate = new Date();
                                 someDate.setDate(someDate.getDate() + counter);
                                 var month = someDate.getUTCMonth() + 1;
                                 var day = someDate.getUTCDate();
                                 var test = month +  "/" + day;
                                 var dayOfWeek = someDate.getDay();
                                 var isWeekend = (dayOfWeek === 6) || (dayOfWeek  === 0);
                                 if (!isWeekend && !holidays.includes(test)) {
                                       daysRemaining--;
                                 }
                                 counter++;
                            }
                            var result = newDate.setDate(newDate.getDate() + counter);
                            $scope.oInfo.caseResubDueDate = newDate;
                            $scope.oInfo.casePrevCMSAnalyst = ObjectModelService.getAssignee($scope.oInfo);
                            ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevAnalyst);
                            ObjectModelService.setGroup($scope.oInfo, 'ALA_ANALYST@'.concat(domain));

                        } else if ($scope.changeCaseStatus.status === "CMS Approved" || $scope.changeCaseStatus.status === "CMS Approved-Documentation Pending") {
                            $scope.oInfo.casePrevCMSAnalyst = ObjectModelService.getAssignee($scope.oInfo);
                            ObjectModelService.setAssignee($scope.oInfo, $scope.oInfo.casePrevAnalyst);
                            ObjectModelService.setGroup($scope.oInfo, 'ALA_ANALYST@'.concat(domain));
                            $scope.oInfo.priority = "N/A";

                        } else if ($scope.changeCaseStatus.status === "CASE_CLOSED" || $scope.changeCaseStatus.status === "Case Closed – MAC Deactivation Referral") {
                              ObjectModelService.setAssignee($scope.oInfo, 'qaassignmentuser@'.concat(domain.toLowerCase()));
                              ObjectModelService.setGroup($scope.oInfo, 'ALA_SUPERVISOR@'.concat(domain));
                              $scope.oInfo.priority = "N/A";

                        }  else if ($scope.changeCaseStatus.status === "Audit Completed" || $scope.changeCaseStatus.status === "Audit N/A") {
                            ObjectModelService.setAssignee($scope.oInfo, 'qacasearchiveuser@'.concat(domain.toLowerCase()));
                            ObjectModelService.setGroup($scope.oInfo, 'ALA_SUPERVISOR@'.concat(domain));
                            $scope.oInfo.priority = "N/A";

                        } else if ($scope.changeCaseStatus.status === "Audit Assigned") {
                            ObjectModelService.setGroup($scope.oInfo, 'ALA_QA_ANALYST@'.concat(domain));
                            $scope.oInfo.priority = "N/A";

                        } else if ($scope.changeCaseStatus.status === "Case Deleted/Canceled") {
                              ObjectModelService.setAssignee($scope.oInfo, 'canceleddeletedcaseu@'.concat(domain.toLowerCase()));
                              ObjectModelService.setGroup($scope.oInfo, 'ALA_SUPERVISOR@'.concat(domain));
                        }

                    $scope.updateParticipants('assignee',ObjectModelService.getAssignee($scope.oInfo));
                    $scope.updateParticipants('owning group',ObjectModelService.getGroup($scope.oInfo));

                    $scope.oInfo.status = $scope.changeCaseStatus.status;

                    if( $scope.loading === true && $scope.displayCMSanalystErrorMessage === false){
                        if((current_assignee ===  ObjectModelService.getAssignee($scope.oInfo)) && (prev_analyst === $scope.oInfo.casePrevAnalyst)) {
                            CaseInfoService.changeCaseFileState('change_case_status', $scope.changeCaseStatus).then(function(data) {
                                    $modalInstance.close();
                            });

                        } else {
                             CaseInfoService.saveCaseInfo(Util.omitNg($scope.oInfo)).then(function(data) {
                                //success
                                CaseInfoService.changeCaseFileState('change_case_status', $scope.changeCaseStatus).then(function(data) {
                                    $modalInstance.close();
                                });
                            });
                        }

                         if($scope.note.note) {
                            ObjectNoteService.saveNote($scope.note).then(function(note) {

                            }, function() {

                            });
                        }
                    }

                    }

                    function cancelModal() {
                        $modalInstance.dismiss();
                    }

                }


                ]);
